...
import DrawerTermsVersions from "@/components/terms-of-use/drawerVersions";
...
export default function PublicTermsOfUse({
  ...
  const getPolicie = async () => {
    setLoading(true);
    try {
      const { data: query }: any = await axios.get(`/api/v1/terms?id=${uuid}`);
      setData(query);
    } catch (error) {
    ...
    <>
      ...
      <div className="w-full max-w-5xl mx-auto py-4 md:py-8 lg:py-12 flex flex-col">
        <div className="flex items-center justify-between pt-4 md:pt-8 px-4">
          <div>
            <div className="text-3xl font-medium">Termos de Uso</div>
        ...
        {data.signHash ? (
          ...
          <>
            <div className="mt-8 pt-8 border-t text-center opacity-70">
              O termos de uso está sendo revisada e logo será publicada.
            </div>
          </>
        )}
      </div>
