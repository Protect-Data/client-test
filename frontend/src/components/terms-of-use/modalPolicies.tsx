...
export default function ModalTerms({
        ...
        setForm({
          ...
          version:
            latestVersion !== "0.0.0"
              ...
              : "1.0.0",
          content: "* INSIRA O CONTEÚDO DOS TERMOS DE USO AQUI *"
        });
      }
    }
  ...
  const handleAdd = async () => {
    ...
    setLoading(true);
    try {
      const { data: query } = await axios.post("/api/v1/terms", {
        ...form
      });
      if (!query.success) {
      ...
      onAdded();
      setOpen(false);
      toast.success("Versão dos Termos de Uso adicionada com sucesso!");
    } catch (error: any) {
  ...
  const handleUpdate = async () => {
    ...
    setLoading(true);
    try {
      const { data: query } = await axios.put("/api/v1/terms", {
        id: edit.id,
        ...form
      });
      if (query.error) {
      ...
      console.error("handleAdd", error);
      toast.error(
        error.message || `Erro ao atualizar versão de termos de uso.`
      );
    } finally {
      setLoading(false);
    }
            ...
            <div className="sm:flex sm:items-start">
              <div className="border-b border-zinc-900/10 pb-12 w-full">
                <h2 className="text-base font-semibold leading-7 text-zinc-900">
                  {edit ? `Editar` : remove ? `Excluir` : `Adicionar`} Termo de
                  Uso
                </h2>
                {remove ? (
                  <>
                    <div className="mt-10 flex flex-col justify-center items-center gap-y-4">
                      ...
                      <span>
                        Deseja excluir a versão dos termos de uso
                        permanentemente ?
                      </span>
                    </div>
                  </>
